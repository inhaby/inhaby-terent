...
export const DesktopHeader: React.FC<DesktopHeaderProps> = ({
  ...
  // Responsive Breakpoints and Priorities
  // - Priority 1 (Always Visible): Logo, Search, Profile, Theme
  // - Priority 2: Messages, Bookings
  // - Priority 3: Visits, Wishlist
  // - Tablet (< 1024px) moves all secondary nav (Priority 2 + 3) into the Profile dropdown.
  // - For Desktop (>= 1024px), dynamic thresholds decide what stays on screen:
  //   * width >= 1280: Show all 4 (Wishlist, Visits, Bookings, Messages)
  //   * width >= 1180: Show Visits, Bookings, Messages (Wishlist overflows)
  //   * width >= 1080: Show Bookings, Messages (Visits, Wishlist overflow)
  //   * width >= 980: Show Messages (Bookings, Visits, Wishlist overflow)
  //   * width < 980 (or tablet): Show 0 secondary nav items on screen

  const isTablet = width < 1024;
  const showWishlist = !isTablet && width >= 1280;
  const showVisits = !isTablet && width >= 1180;
  const showBookings = !isTablet && width >= 1080;
  const showMessages = !isTablet && width >= 980;
      ...
      <div className="max-w-7xl mx-auto h-full px-6 flex items-center justify-between gap-4">
        ...
        <div className="flex-1 px-2 flex justify-center min-w-[280px] lg:min-w-[320px] max-w-[420px] lg:max-w-[520px] mx-auto transition-all duration-350">
          <div 
            ...
            className="w-full relative flex items-center bg-theme-bg hover:bg-theme-bg/90 border border-theme-border focus-within:bg-theme-surface focus-within:border-theme-accent rounded-full py-2 pl-5 pr-2 transition-all duration-300 shadow-inner group cursor-pointer"
          >
            <Search className="text-theme-text-secondary group-focus-within:text-theme-accent stroke-[2.5] flex-shrink-0" size={16} />
            <input 
              type="text" 
              placeholder={`Search verified ${activeCategory === 'all' ? 'homes, rooms, PGs' : activeCategory}...`}
              value={searchQuery}
              onChange={(e) => {
                if (onSearchClick) {
                  onSearchClick();
              ...
              className="w-full bg-transparent border-none text-xs font-semibold placeholder-theme-text-secondary/60 text-theme-text-primary focus:outline-none focus:ring-0 ml-3 py-1 font-sans cursor-pointer animate-in duration-300"
            />
            
            {/* Filters Button is hidden on tablet, visible only on desktop */}
            <button 
              onClick={(e) => {
                e.stopPropagation();
                if (onSearchClick) {
              ...
              className="hidden lg:flex items-center gap-1.5 bg-theme-accent hover:bg-theme-accent-hover text-white font-bold text-[10px] uppercase tracking-wider px-4 py-2.5 rounded-full shadow-md transition-all active:scale-95 cursor-pointer flex-shrink-0 animate-in fade-in"
            >
              <SlidersHorizontal size={12} className="stroke-[2.5]" />
              <span>Filters</span>
            </button>
          </div>
        </div>

        {/* Right Nav Area: Dynamic Responsive Layout */}
        <div className="flex items-center gap-4 shrink-0 font-sans">
          
          {/* Header Navigation Links */}
          {(showMessages || showBookings || showVisits || showWishlist) && (
          ...
          {(showMessages || showBookings || showVisits || showWishlist) && (
            <div className="h-6 w-px bg-theme-border/60"></div>
          )}
              ...
              return (
                <>
                  <button 
                    ...
                    className={`flex items-center gap-2 pr-3.5 pl-1.5 py-1.5 rounded-full border transition-all cursor-pointer ${
                  ...
                  >
                    ...
                    <span className="text-[10px] font-black uppercase tracking-wider">{profileName}</span>
                  ...
                  {isProfileOpen && (
                    <>
                      ...
                      <div className="absolute right-0 mt-3.5 w-64 bg-theme-surface border border-theme-border rounded-2xl shadow-xl py-3 z-50 animate-in fade-in slide-in-from-top-3 duration-250 select-none font-sans text-sans">
                        ...
                        <div className="px-4 py-3 border-b border-theme-border/50 mb-2 flex items-center gap-3">
                        ...
                        <div className="max-h-[380px] overflow-y-auto no-scrollbar font-sans text-xs">
                          ...
                          {/* Section 1: Core Account Settings */}
                          <div className="px-2 py-1 space-y-0.5">
                            <button 
                              onClick={() => { onTabChange('PROFILE'); setIsProfileOpen(false); }}
                              className={`w-full text-left px-3 py-2 text-[11px] font-bold rounded-xl hover:bg-theme-bg/60 uppercase tracking-wider flex items-center gap-2.5 transition-colors ${activeTab === 'PROFILE' ? 'text-theme-accent bg-theme-accent-soft' : 'text-theme-text-primary'}`}
                            >
                            ...
                            <button 
                              ...
                              className={`w-full text-left px-3 py-2 text-[11px] font-bold rounded-xl hover:bg-theme-bg/60 uppercase tracking-wider flex items-center gap-2.5 transition-colors ${activeTab === 'VERIFICATION' ? 'text-theme-accent bg-theme-accent-soft' : 'text-theme-text-primary'}`}
                            >
                          ...
                          {/* Section 2: Bookings and Activity (ONLY shown when responsive overflow is required) */}
                          {(!showBookings || !showWishlist || !showVisits || !showMessages) && (
                            <>
                              <div className="px-2 py-1 space-y-0.5">
                                {!showBookings && (
                                  <button 
          ...
          {/* Theme Switcher on Desktop & Tablet */}
