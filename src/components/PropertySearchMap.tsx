...
export const PropertySearchMap: React.FC<PropertySearchMapProps> = ({
  ...
  if (!hasValidKey) {
    return (
      <div className={`p-6 border border-dashed rounded-3xl flex flex-col justify-center items-center text-center h-full min-h-[350px] font-sans transition-colors ${isDark ? 'bg-zinc-900 border-zinc-700 text-zinc-300' : 'bg-stone-50 border-stone-200 text-stone-700'
        }`}>
            ...
            <React.Fragment key={prop.id}>
              ...
              >
                <div
                  ...
                  className={`px-2.5 py-1.5 rounded-full font-sans font-black text-[10px] md:text-xs transition-all duration-300 shadow-md ${isActive
              ...
              <div className="w-[180px] p-1 font-sans text-stone-900 leading-normal flex flex-col gap-2">
                <div className="relative h-20 rounded-lg overflow-hidden bg-stone-100 flex-shrink-0">
                  {activeProperty.image ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-stone-400 text-[8px] font-bold uppercase">No photo</div>
                  )}
